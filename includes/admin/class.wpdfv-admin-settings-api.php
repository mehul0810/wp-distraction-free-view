		...
		function admin_init() {
			...
			foreach ( $this->fields_array as $section => $field_array ) {
				foreach ( $field_array as $field ) {
					...
					if ( ! empty( $help_link ) ) {
						$name .= sprintf( '&nbsp;<a title="%1$s" class="wpdfv-help-link" href="%2$s" target="_blank">', $name, $help_link );
...
//			$html .= sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id'] );
		...
		function callback_multicheck( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				...
				$html   .= sprintf( '<label for="wpdfv-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html   .= sprintf( '<input type="checkbox" class="checkbox" id="wpdfv-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $checked, $key, false ) );
		...
		function callback_radio( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				$html .= sprintf( '<label for="wpdfv-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html .= sprintf( '<input type="radio" class="radio" id="wpdfv-%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $value, $key, false ) );
				$html .= sprintf( '%1$s</label><br>', $label );
			}
			$html .= $this->get_field_description( $args );
			$html .= '</fieldset>';
		...
		public function settings_page() {
			?>
			<div class="wrap">
				<h2><?php esc_html_e( 'Settings - WP Distraction Free View', 'wpdfv' ); ?></h2>
		...
		public function navigation_html() {
			...
			<h2 class="nav-tab-wrapper">
				...
				foreach ( $this->sections_array as $tab ) {
					...
					?>
					<a href="?page=wp-distraction-free-view&tab=<?php echo $tab['id']; ?>" class="nav-tab <?php echo $active_class; ?>">
						<?php echo $tab['title']; ?>
					</a>
					<?php
				}
